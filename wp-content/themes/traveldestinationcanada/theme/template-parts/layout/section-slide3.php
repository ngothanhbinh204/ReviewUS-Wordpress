...
$items = $cfg['items'] ?? array();
$posts = $cfg['posts'] ?? array(); // Support both 'items' and 'posts'
$loop = $cfg['loop'] ?? false;
$perView = $cfg['perView'] ?? 3;

// Use posts if items is empty (backward compatibility)
if (empty($items) && !empty($posts)) {
	$items = $posts;
}

// Don't display if no title and no items
if (empty($sub_title) && empty($items)) {
	return;
}
?>
    ...
    <div class="c-container px-4 md:px-16 2xl:px-20 3xl:px-0 max-w-screen-2xl 2xl:mx-auto">
        <div class="items-end justify-between md:flex">
            <div class="mb-2 md:mb-0 md:max-w-[70%] xl:max-w-none">
                <?php if (!empty($title)): ?>
                <p class="text-base leading-[26px] xl:text-lg xl:leading-[28px] mb-1 font-semibold">
...
// Optimize navigation button updates
function updateNavButtons3(swiper) {
