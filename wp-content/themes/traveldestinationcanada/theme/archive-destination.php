...
/**
 * Template Name: Page Places To Go
 */
...
$hero_banner_text = get_field('hero_banner_text', 'option');
...
<!-- Basic Content -->
<?php
$intro_content = get_field('intro_content', 'option');
...
<?php
// Get slider items from ACF
$slider_items = get_field('slider_item_places_to_go', 'option');
...
<?php
// Articles & Guides Section - Theme Taxonomy Terms

// Lấy tất cả các terms đã được gán cho bất kỳ bài viết nào
$raw_terms = get_terms([
	'taxonomy' => 'thing_themes',
	'hide_empty' => true, // Chỉ lấy terms có bài viết
]);
...
if (!empty($raw_terms) && !is_wp_error($raw_terms)):
	foreach ($raw_terms as $term) {
		...
		$theme_item->thumbnail = $thumbnail; // Thumbnail từ ACF
